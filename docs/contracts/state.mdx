...
```rust State_Machine
┌─────────┐
│InAuction│ ─┬──►  Failed { NoEnoughBidders }
└─────────┘  │
             │
             │    ┌─────────┐
...
2. If an auction does not have enough bidders, its status becomes [_`Failed { NoEnoughBidders }`_](focus://2[20:46]).
...
6. If more than half of arbitration owners [_`veto`_](focus://15[39:47]) the story, the proposal moves back to the [_`InWriting`_](focus://5[18:29],6[18:29],7[18:29]) state and repeats steps 4-6.
