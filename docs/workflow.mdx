...
- Writer submits a proposal to `improv-hub`; each character in the proposal is associated with a `chain-id` and a list of addresses indicating NFT collections that character can be from
    - *Implementation details*
        - `improv-hub` submits a message to Wormhole that contains information about the proposal:
            - Proposal ID
            - Character IDs, whitelisted NFT collections
            - Auction end time
            - Auction reserve price (minimum bid price)
            - Auction minimum increment (next bidder must be X higher than current bid)
- Bidders on various chains submit a bid to the `improv-auction` contract on their respective chain
    - *Implementation details*
