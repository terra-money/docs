...
A writer submits a proposal to the `improv-hub`. Proposals contain a brief synopsis summarizing their proposed story, a cast of charecters,  Each character in the proposal is associated with a `chain-id` and a list of addresses. This list specifies which NFT collections are elligible for casting. 
