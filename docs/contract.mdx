...
- [claim_all_funds()](#claim_all_funds)
- [change_stream_info()](#change_stream_info)
- [cancel_stream()](#cancel_stream)
- [query_config()](#query_config)
- [nft_to_stream_id()](#nft_to_stream_id)
- [stream_to_nft_id()](#stream_to_nft_id)
...
Create a new stream with the parameters specified in the [`CreateStreamMsg`] struct.

The stream's ID is a unique incrementing number and is returned in the `token_id` attribute.

When creating a stream, you can choose whether it will be `immutable`. Immutable streams cannot be altered, funded, or cancelled once created. 
...
Funding a stream only changes the total amount of tokens that will be transferred and does not change the end date of a stream. Funding a stream increases the number of funds paid per block. 
...
You may specify multiple streams to claim from. The sender of this message must be the owner of any stream that is claimed.
...
Streams can only be canceled by their creator.

If the stream is immutable, it cannot be canceled.
