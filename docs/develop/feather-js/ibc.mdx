...
const tx = await wallet.createAndSignTx({ msgs: [transfer], chainID: 'pisco-1' });
const result = await lcd.tx.broadcast(tx, chainID: 'pisco-1');

console.log(result);
```

Instructions for initializing LCDClient can be found [here](https://docs.terra.money/docs/develop/sdks/feather-js/common-examples.html#configuring-lcdclient).
