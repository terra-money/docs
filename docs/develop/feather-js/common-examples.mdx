...
const balanceTerra = lcd.bank.balance(
  'terra1x46rqay4d3cssq8gxxvqz8xt6nwlz4td20k38v',
);
console.log(balanceTerra);

const balanceOsmo = lcd.bank.balance(
  'osmo1x46rqay4d3cssq8gxxvqz8xt6nwlz4td20k38v',
);
console.log(balanceOsmo);
```
...
If the lcd is configured for multiple chains, you can specify the chainID to query.
In addition, some queries always require a chainID to be specified.
...
const txInfo = await lcd.tx.txInfo(hash, 'pisco-1');
console.log(txInfo);
```
...
const result = await lcd.tx.broadcast(tx, chainID: 'pisco-1');
...
const tx = await wallet.createAndSignTx({ msgs: [cw20Send], chainID: 'pisco-1' });
const result = await lcd.tx.broadcast(tx, chainID: 'pisco-1');
...
const tx = await wallet.createAndSignTx({ msgs: [terraSwap], chainID: 'pisco-1' });
const result = await lcd.tx.broadcast(tx, chainID: 'pisco-1');
...
// sign the tx
wallet
  .createAndSignTx(YOUR_TX_HERE, chainID: YOUR_CHAIN_ID_HERE)
