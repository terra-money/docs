...
You can also create and send transactions to the Terra network while utilizing Wallet Provider. You can use `station.js` to generate a sample transaction:
...
2. Populate `Tx.js` with the following:
   ...
   ```js
   import { Fee, MsgSend } from '@terra-money/station.js';
   import {
     useConnectedWallet,
     UserDenied,
   } from '@terra-money/wallet-provider';
