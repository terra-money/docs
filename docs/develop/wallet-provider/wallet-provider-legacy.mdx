...
# Wallet Provider Legacy 


<Admonition type="tip" icon="🛰" title="Legacy for mobile">

[Wallet Provider Legacy](https://github.com/terra-money/wallet-provider) is for mobile developers for the Station mobile app. It is not intended for interchain functionality, as Station mobile only supports the Terra and Terra Classic chains. For apps needing to connect to interchain Station, use [Wallet Provider](./wallet-provider-tutorial.mdx). 
