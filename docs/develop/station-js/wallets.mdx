...
Use `InterchainLCDClient.wallet()` to create a `Wallet` from a `Key`.

```ts
import { InterchainLCDClient, MnemonicKey } from '@terra-money/station.js';
