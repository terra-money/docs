...
Use the following common examples to learn how to use station.js. If this is your first time using station.js, use the [station.js installation guide](./getting-started.mdx).
...
The following code example shows how to initialize the LCDClient. The rest of the examples assume you initialized it by using this example or similar code. The LCDClient configuration accepts a boolean `legacy` value that is false by default. When set to `true`, the LCD will connect to Terra Classic.
...
```ts
import { LCDClient, MnemonicKey, MsgSend } from '@terra-money/station.js';
...
```ts
import { LCDClient, Tx } from '@terra-money/station.js';
...
// advanced address validation, it also verifies the bech32 checksum
function isValid(address) {
  try {
...
Occasionally the broadcast function of station.js and terra.py will throw the error `Status 500: timed out waiting for tx to be included in a block`. This occurs because the libraries use `broadcast-mode = block` by default, which causes the LCD, to which you are broadcasting the transaction, to send an http response to your request only when the transaction has been included in a block. If the chain is overloaded, the confirmation may take too long and trigger a timeout in the LCD.
