...
When a new module is added to Terra Core, you must also add it to several places in station.js. To add a module, complete the following steps.
...
station.js provides an easy way to generate `ParameterChangeProposal`s, which is a proposal for changing the blockchain parameters associated with a module. If your module has parameters that can be changed via proposal, you should create the following files:
...
## Add API functionality to the InterchainLCDClient

If there are API endpoints that exist for the new module, you will need to add this functionality to `InterchainLCDClient` so that they are accessible.
...
3. Add the functionality to `src/client/lcd/InterchainLCDClient.ts`:
...
export class InterchainLCDClient {
