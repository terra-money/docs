...
This document explains how to perform tasks related to working with smart contracts with station.js.
...
```ts
import { MsgInstantiateContract } from '@terra-money/station.js';
...
A contract can define a query handler, which understands requests for data specified in a JSON message called a QueryMsg. Unlike the message handler, the query handler cannot modify the contract's or blockchain's state -- it is a read-only operation. Therefore, querying data from a contract does not use a message and transaction, but works directly through the `InterchainLCDClient` API.
