...
Terra has full IBC transfer capabilities through both station.js and Terra Station. Although IBC functionality is not readily exposed through Terra Station’s front end, it can be fully incorporated into any dApp. It is up to a dApp’s front end to initiate IBC transfers.

## MsgTransfer

station.js exports a `MsgTransfer` class that can be used to construct IBC transfers.
...
Instructions for initializing LCDClient can be found [here](https://docs.terra.money/docs/develop/sdks/station-js/common-examples.html#configuring-lcdclient).
