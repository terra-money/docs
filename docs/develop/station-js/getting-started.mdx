...
# Get started with station.js

This is an in-depth guide on how to use the `station.js` SDK.
...
By the end of this guide, you'll be able to execute a token swap from your application using station.js.
...
```sh
mkdir my-station-js-project
```

2. Enter your new project directory:

```sh
cd <my-station-js-project>
```

3. Next, initialize npm, install the `station.js` package, and create an `index.js` file to house the code:
...
Terra’s LCD or Light Client Daemon allows users to connect to the blockchain, make queries, create wallets, and submit transactions. It's the main workhorse behind `station.js`.
...
import { Coins, InterchainLCDClient } from '@terra-money/station.js';
const gasPrices = await fetch('https://pisco-api.terra.dev/gas-prices', {
  redirect: 'follow',
});
const gasPricesJson = await gasPrices.json();
const gasPricesCoins = new Coins(gasPricesJson);
const lcd = new InterchainLCDClient({
...
```ts
import { MsgExecuteContract } from '@terra-money/station.js';
...
View the [Common examples](common-examples.mdx) section for more information on using station.js.
