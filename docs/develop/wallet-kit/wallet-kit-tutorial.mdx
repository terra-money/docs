...
This guide will cover how to set up a React app, integrating Wallet Kit, checking the balance of the connected account, and call a token swap. If you want to integrate Station into an existing react app you can skip past the `Project Setup` section.
...
2. Populate `Tx.js` with the following. To make this example interchain retrieve the `baseAsset` from the network object.
   ...
   import { useConnectedWallet, UserDenied } from '@terra-money/wallet-kit';
