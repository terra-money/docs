...
reduced by `SlashFractionDoubleSign`, as defined by the [slashing module](spec-slashing.mdx). The reduction is implemented at the point when the infraction occurred instead of when the evidence was discovered.
