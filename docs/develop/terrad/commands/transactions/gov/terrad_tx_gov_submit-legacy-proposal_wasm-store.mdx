# terrad tx gov submit-legacy-proposal wasm-store

Submit a wasm binary proposal. 

```sh Terrad
terrad tx gov submit-legacy-proposal wasm-store [wasm file] --title [text] --description [text] --run-as [address] --unpin-code [unpin_code] --source [source] --builder [builder] --code-hash [code_hash] [flags]
```
