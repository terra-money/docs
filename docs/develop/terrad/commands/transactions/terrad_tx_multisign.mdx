...
Read one or more signatures from one or more `[signature]` files, generate a multisig signature compliant to the multisig key `[name]`, and attach the key name to the transaction read from `[file]`.

## Example

```sh Terrad
terrad tx multi-sign transaction.json k1k2k3 k1sig.json k2sig.json k3sig.json
...
```sh Terrad
terrad tx multi-sign [file] [name] [[signature]...] [flags]
```
