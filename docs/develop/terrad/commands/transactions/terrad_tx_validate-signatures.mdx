...
This command will check whether all required signers have signed the transactions, whether the signatures were collected in the right order, and if the signature is valid over the given transaction. If the `--offline` flag is also set, signature validation over the
transaction will not be performed as it requires RPC communication with a full node.

## Usage

```sh Terrad
terrad tx validate-signatures [file] [flags]
```
