...
If you use this command with `--broadcast-mode=sync` or `--broadcast-mode=async`, the `max-msgs` flag will automatically be set to `0`.
...
```sh Terrad
terrad tx distribution withdraw-all-rewards --from mykey
...
```sh Terrad
terrad tx distribution withdraw-all-rewards [flags]
```
