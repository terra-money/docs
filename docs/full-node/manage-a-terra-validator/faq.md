...
- **Tendermint Consensus Keypair:** This Consensus Keypair is on the [tendermint](https://docs.tendermint.com/master/nodes/validators.html#validator-keys) layer and consists of a unique Private Key used to sign block hashes associated with a Public Key `terravalconspub`.
  ...
  - The Private Key can be found in the `priv_validator_key.json` file in the `config` directory after runing `terrad init`.
...
:::
::: {warning}
A validator requires all the keys above in order to identify itself on the network, sign blocks, sign oracle-related transactions and sign staking/operational transactions such as voting on Governance proposals. It is the validator's sole responsibility to secure each and every one of these keys and have a contingency backup plan in the event of contingencies.
:::
